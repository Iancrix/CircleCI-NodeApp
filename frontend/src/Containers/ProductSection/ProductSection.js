...
class ProductSection extends Component {
	...
	updateCategories = () => {
		...
		axios
			.get(
				`http://23.99.202.204:5000/productsRoute/${this.props.match.params.productType}/count`
			)
			.then(res => {
				this.setState({
					filterCategories: this.state.filterCategories.map(filterCategory => {
	...
	componentDidUpdate(prevProps) {
		...
		if (locationChanged) {
			axios
				.get(
					`http://23.99.202.204:5000/productsRoute/${this.props.match.params.productType}/filter${this.props.history.location.search}`
				)
				.then(res => {
					this.setState({
